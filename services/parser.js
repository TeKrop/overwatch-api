 ...
 */
ParserService.convertDomToApiData = function($, node) {
    let data = {};
 ...
 */
ParserService.convertNodeAttributes = function($, node) {
    let method = null;
    let params = null;
    let data = null;
 ...
 */
ParserService.configContainsSpecificKey = function(routeConfig, specificKeys) {
    let found = false;
    ...
    for (const key in routeConfig) {
        ...
        if (key === specificKeys[0]) {
            ...
            if (newKeys.length === 0) {
                ...
                found = ParserService.configContainsSpecificKey(routeConfig[key], newKeys);
            }
        } else if (routeConfig[key] instanceof Object) {
            // else, if the value is an object, search recursively
            found = ParserService.configContainsSpecificKey(routeConfig[key], specificKeys);
        }
    }
